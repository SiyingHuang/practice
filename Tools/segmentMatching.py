...
data_section_prov = pd.read_csv(path2, header=None, names=['prov', 'city', 'sec'], dtype={'sec': np.int32})
...
# data['sec'] = data['mobileno'].map(lambda x: str(x)[3:10]).astype(np.int32)  # 提取前7位号段（读入11位号码时）
# (1) 本异网条数统计
